...
# Reshape for convolution recognition: [batch size; height; width; channels]
x_1d_image = tf.reshape(x_1d, [-1, 28, 28, 1])

# Convolutional Layer 1: [height; width; input channels; output channels]
...
with tf.Session() as sess:
    ...
    for i in range(steps_number):
        ...
        print("Accuracy of %s epoch: %s" % (i, epoch_acc))
    ...
    result_acc = sess.run(acc, feed_dict={x_1d: mnist.test.images[indices],
                                          y_: mnist.test.labels[indices],
                                          })

    print("Result accuracy: %s" % result_acc)
